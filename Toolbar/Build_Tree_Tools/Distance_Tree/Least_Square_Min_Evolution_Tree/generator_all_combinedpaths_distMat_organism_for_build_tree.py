...
def fuse_last(all_prepared_path, Adjusted_length_Organism_Set):
    for j in all_prepared_path:
            if j[0][0] == 'X' + str(Adjusted_length_Organism_Set) and j[0][1] == 'X'+str(Adjusted_length_Organism_Set-1):
                w_replace=j[1][0] + j[1][1]
                w_remove=j[1][1]
            if j[0][0] == 'X'+ str(Adjusted_length_Organism_Set):
                j[1].remove(w_remove)
                j[1][0]=w_replace
    return all_prepared_path
...
def generate_all_combinations_of_tree(Organism_Set):
    ...
    result_path_edited=fuse_last(result_path,len(Organism_Set)-1)


    grand_list=replacew_all_combinations(combination_set,result_path_edited)
...
glist=generate_all_combinations_of_tree(organisms)
